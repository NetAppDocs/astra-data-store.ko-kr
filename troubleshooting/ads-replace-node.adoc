...
summary: 노드를 교체하려면 kubeck 명령을 Astra Data Store와 함께 사용하십시오. 
---
= 노드를 교체합니다
:hardbreaks:
:allow-uri-read: 
...
Astra Data Store에 kubtl 명령을 사용하여 클러스터에서 장애가 발생한 노드를 교체합니다.

.단계
. "astrads-system" 네임스페이스의 POD를 나열하십시오(이 예에서는 클러스터에 4개의 노드가 있는 1x5 구성을 사용합니다).
+
[listing]
----
...
----
. 노드 HA가 장애가 발생한 노드에서 "거짓"으로 표시되는지 확인합니다.
+
[listing]
----
...
----
. 클러스터 CR을 수정하여 장애가 발생한 노드를 제거합니다. 노드 수는 3개로 감소합니다.
+
[listing]
----
