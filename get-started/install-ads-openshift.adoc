...
summary: OpenShift를 사용하여 Astra Data Store를 설치하는 방법 
---
...
Red Hat OpenShift Container Platform(OCP)에 Astra Data Store 미리보기를 설치하려면 에서 설치 번들을 다운로드하십시오 https://mysupport.netapp.com/site/products/all/details/astra-data-store/downloads-tab["NetApp Support 사이트"^] 및 이 절차에 설명된 설치 단계를 완료합니다.

.무엇을 &#8217;필요로 할거야
* 설치를 시작하기 전에 link:requirements.html["Astra Data Store 구축을 위한 환경을 준비합니다"].
...
* A link:requirements.html#licensing["NetApp 라이센스 파일"] (NLF) - Astra Data Store 미리 보기. 라이센스 다운로드 지침은 가입 후 발송됩니다.
* 활성 컨텍스트에 대한 클러스터 관리자 권한이 있는 활성 kubecononfig.
* 에 대한 이해 link:faq-ads.html["역할 및 권한"] Astra Data Store 미리 보기에서 사용합니다.
...
----
. 클러스터 YAML이 적용되기 전에 모든 Kubernetes 노드에서 이 절차를 완료하십시오.
...
----
. kubbeck apply 명령을 사용하여 운용자를 배치한다.
+
[listing]
----
...
----
. SELinux가 설정된 경우 Astra Data Store preview 클러스터의 노드에 있는 다음 디렉토리에 대한 'strinux' 컨텍스트의 레이블을 다시 지정합니다.
+
[listing]
----
...
.단계
. NAT 테이블의 출력 체인에 있는 현재 KUBE-HOSTS 규칙의 행 번호를 확인합니다.
+
...
----
. KUBE-HOSTS 항목 바로 위에 NAT 테이블에 새 규칙을 추가합니다.
+
[listing]
----
$ sudo iptables -t nat -I OUTPUT 4 -s 127.0.0.1 -j KUBE-MARK-MASQ -p tcp --dport 9920
----
. 새로 추가된 규칙이 KUBE-HOSTS 규칙 바로 위에 NAT 테이블에 추가되었는지 확인합니다.
+
[listing]
----
$ sudo iptables -t nat -L OUTPUT --line-numbers
...
NOTE: Astra Data Store 미리보기 클러스터의 모든 노드에 대해 단계를 수행합니다.


NOTE: Astra Data Store preview 클러스터 노드가 다시 시작되면 이 단계를 반복합니다.
